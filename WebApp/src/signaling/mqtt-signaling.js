...
export class MQTTSignaling {
	...
	openConnection() {
        if (this.port) {
            this.client = new Paho.Client(this.host, this.port, "client-" + Math.floor(Math.random()*1000000));
        }
        else {
            this.client = new Paho.Client(this.host, "client-" + Math.floor(Math.random()*1000000));
        }
